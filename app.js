...
let pCount=4;function renderParagraphs(){const box=$('paragraphs');const existing=[...box.querySelectorAll('input')].map(x=>x.value);box.innerHTML='';for(let i=0;i<pCount;i++){const d=document.createElement('div');d.className='paragraph-card';d.innerHTML=`<strong>P${i+1}</strong><label>What job does this paragraph perform?<input value="${existing[i]||''}" placeholder="e.g., OLD VIEW → PROBLEM" aria-label="Paragraph ${i+1} role"></label>`;box.appendChild(d)}}$('addParagraph').onclick=()=>{if(pCount<8){pCount++;renderParagraphs()}};$('removeParagraph').onclick=()=>{if(pCount>1){pCount--;renderParagraphs()}};renderParagraphs();
