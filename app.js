...
function saveDraft(){const d={};fields.forEach(id=>{if($(id))d[id]=$(id).value});document.querySelectorAll('[data-write]').forEach(x=>d[x.dataset.write]=x.value);localStorage.setItem('lsat-draft',JSON.stringify(d))}
function restoreDraft(){const d=JSON.parse(localStorage.getItem('lsat-draft')||'{}');Object.entries(d).forEach(([id,v])=>{const el=$(id)||document.querySelector(`[data-write="${id}"]`);if(el)el.value=v})}

document.querySelectorAll('.phase').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.phase').forEach(x=>x.classList.remove('active'));btn.classList.add('active');const p=phases[btn.dataset.phase];$('phaseTitle').textContent=p.title;$('phaseText').textContent=p.text;localStorage.setItem('lsat-phase',btn.dataset.phase)}));
const savedPhase=localStorage.getItem('lsat-phase');if(savedPhase)document.querySelector(`.phase[data-phase="${savedPhase}"]`)?.click();
...
let pCount=4;function renderParagraphs(){const box=$('paragraphs');const existing=[...box.querySelectorAll('input')].map(x=>x.value);box.innerHTML='';for(let i=0;i<pCount;i++){const d=document.createElement('div');d.className='paragraph-card';d.innerHTML=`<strong>P${i+1}</strong><label>What job does this paragraph perform?<input placeholder="e.g., OLD VIEW → PROBLEM" aria-label="Paragraph ${i+1} role"></label>`;d.querySelector('input').value=existing[i]||'';box.appendChild(d)}}$('addParagraph').onclick=()=>{if(pCount<8){pCount++;renderParagraphs()}};$('removeParagraph').onclick=()=>{if(pCount>1){pCount--;renderParagraphs()}};renderParagraphs();
